...
class AVLTree : public Drawable
{
   ...
   public:
      AVLTree(int (*comp_items) (T* item_1, T* item_2), int (*comp_keys) (String* key, T* item), bool minMax, bool allowDuplicates, bool duplicatesOnLeft);
      ~AVLTree();
   ...
   //Pre:Do not call if no items have been entered
   //Post:Returns either min or max value based on minMax value passed in in constructor
      T* remove();
...
T* AVLTree<T>::remove()
{
   ...
   if(min_or_max)
   {
      removedItem = findLeftMost(root);
      root = removeLeftMost(root);
   }
   else
   {
      removedItem = findRightMost(root);
...
template < class T >
AVLTree<T>::AVLTree(int (*comp_items) (T* item_1, T* item_2), int (*comp_keys) (String* key, T* item), bool minMax, bool allowDuplicates, bool duplicatesOnLeft)
{
   root = NULL;
   sze = 0;
   ...
   duplicates_on_left = duplicatesOnLeft;
}
...
AVLTreeNode<T>* AVLTree<T>::removeRightMost(AVLTreeNode<T>* tNode) 
{
   ...
   else 
   {
      ...
      if (avlFlag)
      {
         tNode = avlFixRemoveRight(tNode);  //came from left
      }
      return tNode;
   }  
}
