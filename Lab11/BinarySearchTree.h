...
{
   //DO THIS
   TreeNode<T>* removedItem = removeItem(root,sk);
}
   ...
   if (tNode != NULL)
   {
	 ...
	 return removeNode(tNode);
      ...
      else if (comp < 0)
      {
         ...
         TreeNode<T>* link = removeItem(left,sk);      
         tNode->setLeft(link);
      }
      else
      {
         TreeNode<T>* right = tNode->getRight();
         TreeNode<T>* link = removeItem(right,sk);      
         tNode->setRight(link);
      }
   }
...
T** BinarySearchTree<T>::toArray()
{
   ...
   for (int i = 0; i < sze; i++)
   {
      if (tNode->getLeft() != NULL)
      {
       sub = tNode->removeLeftMost();
      }
      
      bstArray[i] = sub->getItem();
   }
